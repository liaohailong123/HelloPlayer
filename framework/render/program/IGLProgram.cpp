...
IGLProgram::IGLProgram(const char* tag) : logger(tag), _program(0), _rotation(0)
{
    logger.i("IGLProgram::IGLProgram()");
}
...
void IGLProgram::initialized()
{
    if (_program <= 0)
    {
        ...
        logger.i("IGLProgram::initialized _program[%d]", _program);
...
void IGLProgram::setTexture(GLuint *textures, int count)
{
 ...
 * @param data 设置纹理数据
 */
void IGLProgram::setTextureData(std::shared_ptr<TextureData> data)
{
    // do nothing... implement by child class
}
        ...
        if (!compiled)
        {
            ...
            if (infoLen)
            {
                ...
                if (buf)
                {
                    ...
                    logger.i("Could not compile shader %d:\n%s\n", shaderType, buf);
                    free(buf);
                }
        ...
        if (linkStatus != GL_TRUE)
        {
            ...
            if (bufLength)
            {
                ...
                if (buf)
                {
                    ...
                    logger.i("Could not link program:\n%s\n", buf);
                    free(buf);
                }
            }
