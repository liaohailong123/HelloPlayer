...
{
public:
    typedef void (*OnLogPrint)(const char *tag, const char *fmt, va_list ap);
public:
    explicit NHLog() : logPrinter(nullptr) {};

    virtual  ~NHLog() = default;

    // log日志的全局默认tag
    const char *const defaultTag = "NHLog";

    // 外部注册的 log 打印函数
    OnLogPrint logPrinter;
    ...
    void i(const char *tag, const char *format, ...) const
    {
        if (logPrinter != nullptr)
        {
            va_list args;
            va_start(args, format);
            logPrinter(tag, format, args);
            va_end(args);
        }
    }
...
public:
     ...
     */
    void addIdleHandler(const std::shared_ptr<NIdleHandler> &handler);

    /**
     * @param handler 移除掉空闲时处理者
     */
    void removeIdleHandler(const std::shared_ptr<NIdleHandler> &handler);
...
private:
     ...
     */
    void pollOnce(long timeoutMillis);
...
public:
     ...
     * @return true表示入队列成功
     */
    static bool enqueueMessage(NMessageQueue *queue, NMessage *msg, long uptimeMillis);
...
public:
    ...
    /**
     * 启动Handler子线程，并且阻塞时等待子线程内创建好 NHandler
     * 该函数执行完毕后，可以调用 getHandler() 函数获取 NHandler*
     */
    void startSync();
