...
NMessageQueue::NMessageQueue()
        : fd(), idleHandlers(), message(nullptr), quitting(false),
          blocked(false)
{
    memset(fd, 0, NUM(fd));
    init();
...
void NMessageQueue::pollOnce(long timeoutMillis)
{
...
void NMessageQueue::addIdleHandler(const std::shared_ptr<NIdleHandler> &handler)
{
    if (handler == nullptr)
    {
        return;
...
void NMessageQueue::removeIdleHandler(const std::shared_ptr<NIdleHandler> &handler)
{
    if (handler == nullptr)
    {
        return;
    ...
    auto find = std::remove(idleHandlers.begin(), idleHandlers.end(), handler);
    if (find == idleHandlers.begin())
    {
        NHLog::instance()->i("remove failure");
    }
}
...
NMessage *NMessageQueue::next()
{
    ...
    for (;;)
    {
        ...
        {
            pendingIdleHandlerCount = static_cast<int>(idleHandlers.size());
        }
        ...
        // 创建一个副本
        // 使用std::vector替代动态数组
        std::vector<NIdleHandler> pendingIdleHandlers(pendingIdleHandlerCount);
        for (int i = 0; i < pendingIdleHandlerCount; ++i)
        {
            pendingIdleHandlers[i] = *idleHandlers[i].get();
        }
        ...
        for (int i = 0; i < pendingIdleHandlerCount; ++i)
        {
            NIdleHandler idler = pendingIdleHandlers[i];
            ...
            if (!keep)
            {
                ...
                // 不需要保存的空闲任务，加锁移除
                idleHandlers.erase(idleHandlers.begin() + i);
            }
        }
